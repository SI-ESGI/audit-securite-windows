...
function Get-SoftwareAndSecurityAudit {
    ...
    try {
        # Test d'accès au registre en premier
        try {
            $testAccess = Test-Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall"
            Write-Host "DEBUG: Acces au registre Uninstall: $testAccess" -ForegroundColor Cyan
            if (-not $testAccess) {
                Write-Host "ERROR: Impossible d'acceder au registre Uninstall!" -ForegroundColor Red
                return $false
            }
        }
        catch {
            Write-Host "ERROR: Test d'acces au registre a echoue: $_" -ForegroundColor Red
            return $false
        }
        
        # Méthode 1: Registre principal
        $allUninstallItems = @()
        try {
            $allUninstallItems = @(Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*)
            Write-Host "DEBUG: Registre principal - entrées totales: $($allUninstallItems.Count)" -ForegroundColor Cyan
        }
        catch {
            Write-Host "ERROR: Get-ItemProperty (registre principal) a echoue: $_" -ForegroundColor Red
        }
        
        $appsWithDisplayName = @($allUninstallItems | Where-Object { $_.DisplayName -and $_.DisplayName.Trim() -ne "" })
        Write-Host "DEBUG: Applications avec DisplayName (registre principal): $($appsWithDisplayName.Count)" -ForegroundColor Cyan
        
        # Méthode 2: Registre WOW6432Node (applications 32-bit)
        $wow64Apps = @()
        try {
            $wow64UninstallItems = @(Get-ItemProperty HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* -ErrorAction SilentlyContinue)
            if ($wow64UninstallItems) {
                $wow64Apps = @($wow64UninstallItems | Where-Object { $_.DisplayName -and $_.DisplayName.Trim() -ne "" })
                Write-Host "DEBUG: Applications 32-bit (WOW6432Node): $($wow64Apps.Count)" -ForegroundColor Yellow
            }
        }
        catch {
            Write-Host "DEBUG: Impossible de lire WOW6432Node: $_" -ForegroundColor Yellow
        }
        
        # Méthode 3: Win32_Product (plus lent mais plus complet)
        $wmiApps = @()
        try {
            Write-Host "DEBUG: Tentative Win32_Product (peut être lent)..." -ForegroundColor Yellow
            $wmiApps = @(Get-WmiObject -Class Win32_Product -ErrorAction SilentlyContinue | Where-Object { $_.Name })
            Write-Host "DEBUG: Applications via WMI Win32_Product: $($wmiApps.Count)" -ForegroundColor Yellow
        }
        catch {
            Write-Host "DEBUG: Win32_Product a echoue: $_" -ForegroundColor Yellow
        }
        
        # Combinaison de toutes les méthodes
        $allAppsFound = @()
        
        # Ajouter les apps du registre principal
        if ($appsWithDisplayName.Count -gt 0) {
            $allAppsFound += $appsWithDisplayName | Select-Object @{Name='DisplayName';Expression={$_.DisplayName}}, 
                                                                   @{Name='DisplayVersion';Expression={$_.DisplayVersion}}, 
                                                                   @{Name='Publisher';Expression={$_.Publisher}}, 
                                                                   @{Name='InstallDate';Expression={$_.InstallDate}},
                                                                   @{Name='Source';Expression={'Registry64'}}
        }
        
        # Ajouter les apps 32-bit
        if ($wow64Apps.Count -gt 0) {
            $allAppsFound += $wow64Apps | Select-Object @{Name='DisplayName';Expression={$_.DisplayName}}, 
                                                        @{Name='DisplayVersion';Expression={$_.DisplayVersion}}, 
                                                        @{Name='Publisher';Expression={$_.Publisher}}, 
                                                        @{Name='InstallDate';Expression={$_.InstallDate}},
                                                        @{Name='Source';Expression={'Registry32'}}
        }
        
        # Ajouter les apps WMI si les autres méthodes ont échoué
        if ($allAppsFound.Count -eq 0 -and $wmiApps.Count -gt 0) {
            $allAppsFound += $wmiApps | Select-Object @{Name='DisplayName';Expression={$_.Name}}, 
                                                      @{Name='DisplayVersion';Expression={$_.Version}}, 
                                                      @{Name='Publisher';Expression={$_.Vendor}}, 
                                                      @{Name='InstallDate';Expression={$_.InstallDate}},
                                                      @{Name='Source';Expression={'WMI'}}
        }
        
        # Dédoublonner par nom d'application
        $uniqueApps = @()
        $seenNames = @{}
        foreach ($app in $allAppsFound) {
            if (-not $seenNames.ContainsKey($app.DisplayName)) {
                $uniqueApps += $app
                $seenNames[$app.DisplayName] = $true
            }
        }
        
        Write-Host "DEBUG: Total applications uniques trouvées: $($uniqueApps.Count)" -ForegroundColor Green
        
        # Trier et limiter aux 50 plus récentes
        $installedApps = @($uniqueApps | Sort-Object -Property InstallDate -Descending | Select-Object -First 50)
        $totalAppsCount = $uniqueApps.Count
        
        Write-Host "DEBUG: Applications sélectionnées pour le rapport: $($installedApps.Count)" -ForegroundColor Green
        Write-Host "DEBUG: Nombre total d'applications (TotalInstalledAppsCount): $totalAppsCount" -ForegroundColor Green
        ...
        # Debug final: Afficher les données stockées
        Write-Host "DEBUG: Verification des donnees stockees:" -ForegroundColor Magenta
        Write-Host "DEBUG: TotalInstalledAppsCount dans auditResults = $($script:auditResults.SoftwareAndSecurity.TotalInstalledAppsCount)" -ForegroundColor Magenta
        Write-Host "DEBUG: RecentInstalledApps count = $($script:auditResults.SoftwareAndSecurity.RecentInstalledApps.Count)" -ForegroundColor Magenta
        
        # Afficher les 5 premières applications trouvées
        if ($script:auditResults.SoftwareAndSecurity.RecentInstalledApps -and $script:auditResults.SoftwareAndSecurity.RecentInstalledApps.Count -gt 0) {
            Write-Host "DEBUG: Premieres applications trouvees:" -ForegroundColor Cyan
            $script:auditResults.SoftwareAndSecurity.RecentInstalledApps | Select-Object -First 5 | ForEach-Object {
                Write-Host "  - $($_.DisplayName) (v$($_.DisplayVersion)) [$($_.Source)]" -ForegroundColor White
            }
        } else {
            Write-Host "DEBUG: AUCUNE APPLICATION TROUVEE - Toutes les méthodes ont échoué!" -ForegroundColor Red
        }
